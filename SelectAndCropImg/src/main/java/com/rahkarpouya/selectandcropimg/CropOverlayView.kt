...
class CropOverlayView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) :
            ...
            if (mCropShape != cropShape) {
                ...
                if (Build.VERSION.SDK_INT <= 17) {
                    if (mCropShape == CropShape.OVAL) {
                        ...
                        if (mOriginalLayerType != LAYER_TYPE_SOFTWARE) { // TURN off hardware acceleration
                            setLayerType(LAYER_TYPE_SOFTWARE, null)
                        } else {
                            mOriginalLayerType = null
                        }
                    } else if (mOriginalLayerType != null) { // return hardware acceleration back
    ...
    private fun initCropWindow() {
        ...
        if (mInitialCropWindowRect.width() > 0 && mInitialCropWindowRect.height() > 0) { // Get crop window position relative to the displayed image.
            rect.left =
...
// then the image height is the determining initial length. Else, vice-versa.
        ...
        } else { // Initialize crop window to have 10% padding w/ respect to image.
            rect.left = leftLimit + horizontalPadding
            rect.top = topLimit + verticalPadding
            rect.right = rightLimit - horizontalPadding
            rect.bottom = bottomLimit - verticalPadding
    ...
    private fun drawBorders(canvas: Canvas) {
        if (mBorderPaint != null) {
            ...
            val rect = mCropWindowHandler.rect
            rect!!.inset(w / 2, w / 2)
            if (mCropShape == CropShape.RECTANGLE) { // Draw rectangle crop window border.
                canvas.drawRect(rect, mBorderPaint!!)
            } else { // Draw circular crop window border
