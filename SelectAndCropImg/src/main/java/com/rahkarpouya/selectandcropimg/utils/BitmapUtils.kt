...
internal object BitmapUtils {
    ...
    fun decodeSampledBitmap(
        ...
        return try {
            ...
            if (options.outWidth == -1 && options.outHeight == -1) throw RuntimeException(
    ...
    private fun cropBitmapObjectWithScale(
        ...
        flipHorizontally: Boolean,
        flipVertically: Boolean
    ): Bitmap? { // get the rectangle in original image that contains the required cropped area (larger for non
// rectangular crop)
        ...
        if (result == bitmap) { // corner case when all bitmap is selected, no worth optimizing for it
            result = bitmap.copy(bitmap.config, false)
        }
        // rotating by 0, 90, 180 or 270 degrees doesn't require extra cropping
        if (degreesRotated % 90 != 0) { // extra crop because non rectangular crop cannot be done directly on the image without
// rotating first
    ...
    fun cropBitmap(
        ...
        var sampleMulti = 1
        while (true) {
            try { // if successful, just return the resulting bitmap
                return cropBitmap(
                    context,
                    loadedImageUri,
                    points,
            ...
            } catch (e: OutOfMemoryError) { // if OOM try to increase the sampling to lower the memory usage
                sampleMulti *= 2
                if (sampleMulti > 16) {
                    throw RuntimeException(
    ...
    private fun cropBitmap(
        ...
        flipVertically: Boolean,
        sampleMulti: Int
    ): BitmapSampled { // get the rectangle in original image that contains the required cropped area (larger for non
// rectangular crop)
        val rect = getRectFromPoints(
            points,
            orgWidth,
        ...
        var sampleSize = 1
        try { // decode only the required image from URI, optionally sub-sampling if reqWidth/reqHeight is
// given.
        ...
        return if (result != null) {
            try { // rotate the decoded region by the required amount
...
// rotating first
        ...
        } else { // failed to decode region, may be skia issue, try full decode and then crop
            cropBitmap(
                context,
                loadedImageUri,
                points,
    ...
    private fun cropBitmap(
        ...
        try {
            ...
            if (fullBitmap != null) {
                try { // adjust crop points by the sampling because the image is smaller
    ...
    private fun cropForRotatedImage(
        ...
        if (degreesRotated % 90 != 0) {
            ...
            val bitmapTmp = bit
            bit = Bitmap.createBitmap(bit!!, rect.left, rect.top, rect.width(), rect.height())
            if (bitmapTmp != bit) {
                bitmapTmp!!.recycle()
            }
        }
    ...
    private fun rotateAndFlipBitmapInt(
        ...
        return if (degrees > 0 || flipHorizontally || flipVertically) {
            ...
            matrix.postScale(
                if (flipHorizontally) (-1).toFloat() else 1.toFloat(),
                if (flipVertically) (-1).toFloat() else 1.toFloat()
            )
...
// Safe minimum default size
    ...
    private val maxTextureSize: Int
        get() { // Safe minimum default size
            val imageMaxBitmapDimension = 2048
            return try { // Get EGL Display
